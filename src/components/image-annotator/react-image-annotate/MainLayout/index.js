...
export const MainLayout = ({ state, dispatch, RegionEditLabel, onRegionClassAdded }) => {
  ...
  const canvas = (
    <ImageCanvas
      {...settings}
      showCrosshairs={
        settings.showCrosshairs && !["select", "pan", "zoom"].includes(state.selectedTool)
      }
      ...
      videoTime={
        state.annotationType === "image" ? state.selectedImageFrameTime : state.currentVideoTime
      }
    ...
    <FullScreenContainer>
      ...
      >
        <HotkeyDiv
          ...
          className={clsx(classes.container, fullScreenHandle.active && "Fullscreen")}
        >
          <Workspace
            allowFullscreen
            iconDictionary={iconDictionary}
            // headerItems={[
            //   fullScreenHandle.active ? { name: "Window" } : { name: "Fullscreen" }
            // ].filter(Boolean)}
            ...
            rightSidebarItems={[
              debugModeOn && <DebugBox state={debugModeOn} lastAction={state.lastAction} />,
              state.taskDescription && <TaskDescription description={state.taskDescription} />,
              state.labelImages && (
                <TagsSidebarBox
