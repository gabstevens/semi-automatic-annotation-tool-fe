...
import { moveRegion } from "../../ImageCanvas/region-tools";
...
export default (state, action) => {
  ...
  switch (action.type) {
    ...
    case "SELECT_IMAGE": {
      return setIn(setIn(state, ["images"], [action.image]), ["selectedImage"], 0);
    }
    case "CHANGE_REGION": {
    ...
    case "MOUSE_MOVE": {
      ...
      switch (state.mode.mode) {
        ...
        case "RESIZE_KEYPOINTS": {
          ...
          const distFromCenter = Math.sqrt((centerX - x) ** 2 + (centerY - y) ** 2);
        ...
        case "DRAW_POLYGON": {
          ...
          return setIn(
            state,
            [...pathToActiveImage, "regions", regionIndex, "points", region.points.length - 1],
            [x, y]
          );
        }
    ...
    case "MOUSE_DOWN": {
      ...
      switch (state.selectedTool) {
        ...
        case "create-keypoints": {
          ...
          const [[keypointsDefinitionId, { landmarks, connections }]] = Object.entries(
      ...
      const regions = [...(getIn(state, pathToActiveImage).regions || [])]
        .map(r => setIn(setIn(r, ["editingLabels"], false), ["highlighted"], false))
      ...
      switch (state.mode.mode) {
        ...
        case "DRAW_EXPANDING_LINE": {
          ...
          if (mouseDistFromLastPoint > 0.002) {
            ...
            newExpandingLine = setIn(expandingLine, ["points"], fixTwisted(newPoints));
          } else {
            return state;
          }
          return setIn(state, [...pathToActiveImage, "regions", regionIndex], newExpandingLine);
        }
        default:
          return state;
      }
    ...
    case "SELECT_TOOL": {
      ...
      if (action.selectedTool === "show-mask") {
